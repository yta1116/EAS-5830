...
def build_merkle(leaves):
    ...
    while len(tree[-1]) > 1:
        ...
        for i in range(0, len(current_layer), 2):
            next_layer.append(hash_pair(int.from_bytes(current_layer[i],'big'), int.from_bytes(current_layer[i+1],'big')))
...
def prove_merkle(merkle_tree, random_indx):
    ...
    while layer_index < len(merkle_tree) - 1:
        layer = merkle_tree[layer_index]
        if leaf_index % 2 == 0:
            if leaf_index + 1 < len(layer):
                merkle_proof.append(layer[leaf_index + 1])
        else:
            merkle_proof.append(layer[leaf_index - 1])
...
def sign_challenge(challenge):
    ...
    # TODO YOUR CODE HERE
    eth_sig_obj = acct.sign_message(challenge)
...
def send_signed_msg(proof, random_leaf):
    ...
    tx = contract.functions.claim(random_leaf, proof).buildTransaction({
        'from': acct.address,
